...
class TruckNode:
	...
	def updateCache(self, node):
		if fabs(self.xDistance(node)) < COMM_DIST:
	...
	'''
	This method cleans out old cache entries.
	'''
	def cleanCache(self):
		for id in self.cache.keys():
			if self.seqNum - self.cache[id].seqNum > OLD_SEQ_DIFF:
				#print "Removed outdated cache entry for", id, "at", self.time % EPOCH_MOD
				#print "\tMy seqNum:", self.seqNum, "Their seqNum:", self.cache[id].seqNum
				del self.cache[id]
				if self.leader == id:
					self.leader = 0
					#print "\tReset leader"
	...
	def updateLeader(self, node):
		...
		if node and node.x[Y] == self.x[Y] and node.x[X] > self.x[X]:
			...
			elif self.cache[self.leader].x[X] > node.x[X]:
				self.leader = node.id
				print "Set leader to", self.leader, "at", self.time % EPOCH_MOD
	...
	def updateFollower(self, node):
		...
		if node and node.x[Y] == self.x[Y] and node.x[X] < self.x[X]:
			...
			elif self.cache[self.follower].x[X] < node.x[X]:
				self.follower = node.id
				print "Set follower to", self.follower, "at", self.time % EPOCH_MOD
	...
	def adjustSelf(self):
		...
		if self.isNotInPlatoon():
			...
			if self.x[Y] == LEFT:
				...
				#I think this may be unreachable given the current conditions for isPlatoonLeader()
	...
	def avoidCollision(self, immLead):
		...
		if d < PLAT_DIST[0]:
			...
			self.accelerate((immLead.v[X] + immLead.a[X] - self.v[X] - FALL_BACK, self.a[Y]))
		elif d > PLAT_DIST[1]:
			self.adjustmentMade = 1
