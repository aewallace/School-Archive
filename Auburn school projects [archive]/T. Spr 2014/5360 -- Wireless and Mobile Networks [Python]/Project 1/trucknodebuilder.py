...
class TruckNodeBuilder:
    ...
    def __init__(self, configFileName, expectedUDPPort):
        ...
        self.collectionOfNeighborsUDPVal = []
        ...
        self.readFromConfigFile(expectedUDPPort, True)
        
        
    def readFromConfigFile(self, expectedUDPPort, doUpdateInternalState):
        ...
        try:
            ...
            self.nodeValueBeingChecked = str(expectedUDPPort)
    ...
    def returnSethsDesire(self):
        #say current X, current Y
        #find all neighbors: search for node M, node N, node 0, etc, and gather exactly what neighbors [port IDs] they can be found at.
        collectionOfNeighborsNN = []
        #self.collectionOfNeighborsUDPVal = []
        for nodeNumber in self.listOfNeighborNodes:
            collectionOfNeighborsNN.append('Node ' + str(nodeNumber))
        for line in self.configFileContents:
            for nodeSearchString in collectionOfNeighborsNN:
                tempLineSplit = []
                if (-1 != line.find(nodeSearchString)):
                    tempLineSplit = line.split()
                    self.collectionOfNeighborsUDPVal.append(int(tempLineSplit[3]))
                    break
                else:
                    tempLineSplit = tempLineSplit
        print self.collectionOfNeighborsUDPVal
