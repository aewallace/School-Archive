...
class TruckNode:
	...
	def blanketSendData(self):
		self.vc.blanketBroadcast(self.seqNum, self.id, self.platId, self.immPlatoonLeader(), self.immPlatoonFollower(), self.x, self.v, self.a)
	...
	def adjustSelf(self):
		...
		if self.isNotInPlatoon() or self.isJoiningPlatoon():
