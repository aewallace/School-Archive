...
class TruckNodeBuilder:
	def __init__(self, id, v=None, path='config.txt', vpath='velocity.txt'):
		self.id = id
		self.v = v
		self.inV = None
		...
		if vpath: #if we have an external path for the velocity file
			try:
				self.velocityFile = open(vpath, 'r')
				self.velocityLines = [line.split() for line in self.velocityFile.readlines()]
				for line in self.velocityLines:
					if int(line[ID]) == self.id:
						self.inV = int(line[VEL])
						print "Using velocity detected in file."
						break
			except IOError:
				print "No velocity file detected; using alt means of setting V."
	...
	def buildNode(self):
		...
		if self.v is not None and self.v >= 0:
			tn.v = (self.v, 0)
		if self.inV is not None and self.inV >= 0:
			tn.v = (self.inV, 0)
		return tn
