...
DROP_PER = 1
#Used to index self's sent packets for end-end time determination
SENT, RECVD = 0, 1
...
class TruckNode:
        ...
        while time() < self.startTime + TIME_LIM:
            ...
            try:
                ...
                if self.bcastCount == RANGE_BCAST_PER / BCAST_PER:
                    ...
                    self.vc.clearFIDCache()
                self.nextTimestep()
            except Exception as e:
                print "Exception in control thread:"
                print e
        self.logFile.close()
        #print "this should break the code", float(3/0)
        if (self.vc.drops + self.vc.recvs) != 0:
        	self.getTurnAroundTime()
        	print "Drop rate:", float(self.vc.drops) / float(self.vc.drops + self.vc.recvs)
        	#print "Total packets detected:", self.vc.drops + self.vc.recvs
        	#print "Total packets received:", self.vc.recvs
        	print "Throughput, packets/s:", float(self.vc.recvs) / TIME_LIM;
        	#print "Packets heard from self:", self.vc.heardFromSelf
        	#print "Packets sent from self:",self.vc.sentFromSelf
        	#print "Known forwards:",self.vc.knownGoodForwards
        	#print "Neighbors:",self.neighbors
        else:
        	print "...But we didn't sense ANY packets! [Did you form your packets correctly?]"
    ...
    def listenerThread(self):
            while time() < self.startTime + TIME_LIM:
                try:
                    ...
                    print "Exception in listener thread:"
                    print e
    ...
    def sendData(self):
        ...
        for neighbor in list(self.neighbors):
            self.vc.sendData(neighbor, self.seqNum, self.id, self.id, self.platId, self.immPlatoonLeader(), self.immPlatoonFollower(), self.sendX, self.sendV, self.sendA, self.neighbors)
            self.vc.forwardPackets(neighbor, self.id, self.x[X], self.neighbors, self.cache)
        self.vc.clearTBF()
        self.bcastCount += 1
    ...
    def blanketSendData(self):
        self.vc.blanketBroadcast(self.seqNum, self.id, self.platId, self.immPlatoonLeader(), self.immPlatoonFollower(), self.sendX, self.sendV, self.sendA, self.neighbors)
    ...
    def nodeFromPacket(self, pkt):
        ...
        for nbr in range(NBR0,NBR9+1):
            if pkt[nbr] in PORT_RANGE:
                node.neighbors.append(pkt[nbr])
        return node
