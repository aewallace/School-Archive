...
Nov 17, start 7:47; pause 9:06; resume 9:08

@author: aew0024
'''
import unittest
import CA08.prod.participant as Participant
import CA08.prod.dressagescore as DressageScore
...
class Test(unittest.TestCase):
    ...
    def test001a0initializationVerification(self):
        ...
        self.assertRaises(ValueError, self.mcflyDS.getDressageScore, None)
        for x in range(0, 20):
            self.mcflyDS.setMovementScore(x+1, 5)
            
        self.assertTrue(self.mcflyDS.setComplete())
        self.assertRaises(ValueError, self.mcflyDS.setComplete, None)
        self.assertTrue(self.mcflyDS.isComplete())
        self.assertRaises(ValueError, self.mcflyDS.setEliminated, None)
        self.assertRaises(ValueError, self.mcflyDS.setWithdrawn, None)
        self.assertRaises(ValueError, self.mcflyDS.setRetired, None)
    ...
    def test003c0canEnterGoodValues(self):
        ...
        self.assertEquals(5, self.mcflyDS.setMovementScore(movement = 5, 5))
        self.assertRaises(ValueError, self.mcflyDS.getDressageScore, None)
        self.assertRaises(ValueError, self.mcflyDS.getEventingDressageScore, None)
        for x in range(5, 20):
        ...
        self.assertEquals((1, "pretty ugly"), self.mcflyDS.eachMovement())
        self.assertEquals((2, "getting better, but still bad"), self.mcflyDS.eachMovement())
        self.assertEquals((3, ""), self.mcflyDS.eachMovement())
        self.assertEquals((4, ""), self.mcflyDS.eachMovement())
        self.assertEquals((5, ""), self.mcflyDS.eachMovement())
    ...
    def test009a1retrieveScoresAfterGoodEntry(self):
        self.assertRaises(ValueError, self.mcflyDS.eachWeightedMovement, None)
        self.assertRaises(ValueError, self.mcflyDS.eachMovement, None)
        self.assertRaises(ValueError, self.mcflyDS.setMovementScore, 0, 5)
        for x in range(0, 20):
            self.mcflyDS.setMovementScore(x+1, 8.5)
        self.assertTrue(self.mcflyDS.setComplete())
        for y in range(0, 20):
            self.assertEquals((8.5, ""), self.mcflyDS.eachMovement())
        for z in range(0, 20):
            (number, comm) = self.mcflyDS.eachWeightedScore()
    ...
    def test010b1invalidScoreEntryBadParams(self):
        self.assertRaises(ValueError, self.mcflyDS.setMovementScore, None)
        self.assertRaises(ValueError, self.mcflyDS.setMovementScore, 1)
    ...
    def test011c1recoverStoredDressageScore(self):
        ...
        self.assertIsInstance(DressageScore.DressageScore, WaffleDressageScore)
        recoveredScoreFromWaffle = Waffle.getDressageScore
        self.assertIsInstance(DressageScore.DressageScore, recoveredScoreFromWaffle)
    ...
    def test015a2setEliminatedThenTryToManipulate(self):
        self.assertFalse(self.mcflyDS.isEliminated())
        self.assertEquals(True, self.mcflyDS.setEliminated())
        set.assertTrue(self.mcflyDS.isEliminated())
        self.assertTrue(self.mcflyDS.setEliminated())
