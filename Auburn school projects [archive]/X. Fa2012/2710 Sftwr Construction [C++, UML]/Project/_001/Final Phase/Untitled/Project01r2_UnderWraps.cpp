...
public:
	SystemLogin();
	~SystemLogin();
	bool getCheckCredentials(vector<Staff>& staffDB, bool& asAdmin);
private:
	Staff* currentStaff;
};
...
public:
	...
	~ClientDataBase();
	bool writeCDB();
	bool addClient(vector<Account>& acDB);
	//vector<int> deleteClient();
	bool editClient(vector<Account>& acDB);
...
public:
	...
	int addAccount(vector<Client>& cdBIn);
	~AccountDataBase();
	bool writeADB();
	//int deleteAccount();
	int editAccount();
...
BankSystem::BankSystem() {
	staffDB.push_back(Staff(true, "0000", "admin"));
}
BankSystem::~BankSystem(){
	delete [] menuOptions;
}
bool BankSystem::initialize(){
	...
	mainMenu.setOptions(input, numToPass);
}
...
bool SystemLogin::getCheckCredentials(vector<Staff>& staffDB, bool& asAdmin){
	...
	else if(indexToCheck > -1 && !staffDB[indexToCheck].isLockedCheck())
	{
		while (notSuccessfulAtPass || tries <= maxTries) {
			...
			cout << "\n";
			if(!currentStaff.checkPassword(tempPass))
			{
				tries++;
